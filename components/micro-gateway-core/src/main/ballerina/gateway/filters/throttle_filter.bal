...
public type ThrottleFilter object {
    ...
    public function doFilterRequest(http:Listener listener, http:Request request, http:FilterContext context) returns
            boolean {
        worker w1{
            //worker thread to carry on API request flow
            io:println("\nstarting throttle filter");
            ...
            //Sends throttleEvent to worker thread 2.
            throttleEvent -> w2;
            io:println("worker 1");
        ...
        worker w2{
            //Worker thread which carry on throttle event flow.
            io:println("worker 2");
            RequestStreamDTO throttleEvent;
            throttleEvent <- w1;
            io:println("worker 22");

            //publish non throttled event.
